...
export const NewAggregationColumn = ({ isOpen, onClose, onSubmit, selectedColumns }: NewAggregationColumnProps) => {
  const [columnName, setColumnName] = useState("New Column");
  const [expression, setExpression] = useState(selectedColumns.map(col => "mean("+col+")").join("+") || "");
  const [nameError, setNameError] = useState("");
  const [expressionError, setExpressionError] = useState("");

  useEffect(() => {
    if(isOpen) {
      setExpression(selectedColumns.map(col => "mean("+col+")").join("+") || "");
    ...
    >
      <div className={Classes.DIALOG_BODY}>
        ...
        <div style={{ paddingTop: 10 }}>
          ...
          columns: <strong>{selectedColumns}</strong>
        </div>
        <div style={{ paddingTop: 10 }}>
